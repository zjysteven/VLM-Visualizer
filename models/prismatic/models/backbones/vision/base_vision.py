...
class TimmViTBackbone(VisionBackbone, ABC):
    def __init__(
        ...
        # Set forward hooks for attention tracking
        self.attention_module_names = []
        for name, module in self.featurizer.named_modules():
            # hard-coded layer name for ViT
            if "attn_drop" in name:
                module.register_forward_hook(self.get_attention)
                self.attention_module_names.append(name)
        assert len(self.attention_module_names) > 0, "No attention modules found in the model"
        self.attention_list = []

    def get_attention(self, module, input, output):
        self.attention_list.append(output.cpu())
    ...
    def forward(self, pixel_values: Union[torch.Tensor, Dict[str, torch.Tensor]]) -> torch.Tensor:
        ...
        self.attention_list = []
        output = self.featurizer(pixel_values)
        assert len(self.attention_list) == len(self.attention_module_names), "Attention list length mismatch"
        self.attentions = OrderedDict(zip(self.attention_module_names, self.attention_list))
        return output
